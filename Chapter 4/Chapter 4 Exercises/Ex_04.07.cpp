...
constexpr int limit = 10;			// a digit must be less than this value
...
int main()
{
	...
	while (1) {										// request user input for first digit			
		...
		cin >> val1;
		if ( (num1=inputCheck(val1)) == limit)		// call function inputCheck() to check if input is a digit
	...
	while (1) {										// request user input for second digit
		...
		cin >> val2;
		if ( (num2=inputCheck(val2)) == limit)		// call function inputCheck() to check if input is a digit
			cout << "That is not a digit! ";
		else
			break;
	}
